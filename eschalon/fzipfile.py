...
Modified from the original Python 2.6.5 Lib/zipfile.py to inline
the decryption function (and unroll it), which results in about a
40% speedup.
        ...
        if bytesToRead > 0:
            ...
            if self.rawbuffer:
                ...
                if newdata and self.decrypter is not None:
                    # Original single line of code here:
                    #newdata = ''.join(map(self.decrypter, newdata))

                    # Faster but still pure Python:
                    ...
                    for c in newdata:
                        ...
                        key0 = ((key0 >> 8) & 0xffffff) ^ crctable[(
                            key0 ^ cord) & 0xff]
                        key1 = (key1 + (key0 & 255)) & 4294967295
                        key1 = (key1 * 134775813 + 1) & 4294967295
                        key2 = ((key2 >> 8) & 0xffffff) ^ crctable[(
                            key2 ^ ((key1 >> 24) & 255)) & 0xff]
